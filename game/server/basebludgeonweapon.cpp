...
void CBaseHLBludgeonWeapon::Hit( trace_t &traceHit, Activity nHitActivity, bool bIsSecondary )
{
	...
	if (pHitEntity->IsNPC()) {
	/*	if (pPlayer->m_killTally >= 1) {
			TriggerTakedown(traceHit);
			pPlayer->m_killTally = 0;
		}*/
		TriggerTakedown(traceHit);
	}
...
//Telvy - wip function to test getting the targetname of an entity. afterwards we can return the data to whatever
string_t  CBaseHLBludgeonWeapon::GetTakedownTarget(trace_t& traceHit)
{
	CBaseEntity* pHitTarget = traceHit.m_pEnt;
	string_t targetname = pHitTarget->GetEntityName();

	return targetname;

}

void CBaseHLBludgeonWeapon::TakedownTarget(trace_t& traceHit) 
{
	CBaseEntity* pHitEntity = traceHit.m_pEnt;
	string_t target = pHitEntity->GetEntityName();
	Warning("%s\n", target);
}

//void CBaseHLBludgeonWeapon::TriggerTakedown(trace_t& traceHit)
//{
//	const char* zombiestr = "npc_zombie";
//	char* soldierstr = "npc_combine_s";
//	char* metrostr = "npc_metropolice";
//
//	CBaseEntity* pHitTarget = traceHit.m_pEnt;
//	const char* targetname = pHitTarget->GetClassname();
//
//	DevMsg("%s\n", targetname);
//
//	CBaseEntity* pResult;
//
//	if (!Q_strcmp(targetname, zombiestr))
//	{
//		pResult = gEntList.FindEntityByName(NULL, "trigger_takedown_zombie");
//
//		if (!pResult)
//			return;
//
//		CLogicRelay* zTriggerRelay = dynamic_cast<CLogicRelay*>(pResult);
//
//		if (zTriggerRelay)
//			zTriggerRelay->m_OnTrigger.FireOutput(this, this);
//	}
//
//	else if (!Q_strcmp(targetname, soldierstr))
//	{
//		pResult = gEntList.FindEntityByName(NULL, "trigger_takedown_soldier");
//
//		if (!pResult)
//			return;
//
//		CLogicRelay* sTriggerRelay = dynamic_cast<CLogicRelay*>(pResult);
//
//		if (sTriggerRelay)
//			sTriggerRelay->m_OnTrigger.FireOutput(this, this);
//	}
//
//	else if (!Q_strcmp(targetname, metrostr))
//	{
//		pResult = gEntList.FindEntityByName(NULL, "trigger_takedown_metrocop");
//
//		if (!pResult)
//			return;
//
//		CLogicRelay* mTriggerRelay = dynamic_cast<CLogicRelay*>(pResult);
//
//		if (mTriggerRelay)
//			mTriggerRelay->m_OnTrigger.FireOutput(this, this);
//	}
//	else {
//		return;
//	}
...
void CBaseHLBludgeonWeapon::TriggerTakedown(trace_t& traceHit)
{
	...
	if (pHitTarget->ClassMatches("npc_zombie"))
	{
		pResult = gEntList.FindEntityByName(NULL, "trigger_takedown_zombie");
	...
	else if (pHitTarget->ClassMatches("npc_nightmare")) 
	{
		pResult = gEntList.FindEntityByName(NULL, "trigger_takedown_nightmare");

		if (!pResult)
			return;

		CLogicRelay* nTriggerRelay = dynamic_cast<CLogicRelay*>(pResult);

		if (nTriggerRelay)
			nTriggerRelay->m_OnTrigger.FireOutput(this, this);
	}

	else if (pHitTarget->ClassMatches("npc_combine_s"))
	{
		pResult = gEntList.FindEntityByName(NULL, "trigger_takedown_soldier");
	...
	else if (pHitTarget->ClassMatches("npc_metropolice"))
	{
		pResult = gEntList.FindEntityByName(NULL, "trigger_takedown_metrocop");
	...
	else if (pHitTarget->ClassMatches("npc_gangster"))
	{
		pResult = gEntList.FindEntityByName(NULL, "trigger_takedown_gangster");

		if (!pResult)
			return;

		CLogicRelay* gTriggerRelay = dynamic_cast<CLogicRelay*>(pResult);

		if (gTriggerRelay)
			gTriggerRelay->m_OnTrigger.FireOutput(this, this);
	}
	else {
		return;
	}

...
void CInput::CAM_Think(void)
{
	...
	if (m_fCameraMovingWithMouse)
	{
		...
		if (!m_fCameraDistanceMove)
		{
			...
			if (m_nCameraX > x)
			{
				...
				{
					idealAngles[YAW] += (CAM_ANGLE_MOVE) * ((m_nCameraX - x) / 2.0f);
				}
				if (idealAngles[YAW] > c_maxyaw.GetFloat())
				{
			...
			else if (m_nCameraX < x)
			{
				...
				{
					idealAngles[YAW] -= (CAM_ANGLE_MOVE) * ((x - m_nCameraX) / 2.0f);
			...
			if (m_nCameraY > y)
			{
				...
				{
					idealAngles[PITCH] += (CAM_ANGLE_MOVE) * ((m_nCameraY - y) / 2.0f);
				}
				if (idealAngles[PITCH] > c_maxpitch.GetFloat())
				{
			...
			else if (m_nCameraY < y)
			{
				...
				{
					idealAngles[PITCH] -= (CAM_ANGLE_MOVE) * ((y - m_nCameraY) / 2.0f);
				}
				if (idealAngles[PITCH] < c_minpitch.GetFloat())
				{
	...
	if (m_fCameraDistanceMove)
	{
		...
		if (m_nCameraY > y)
		{
			...
			{
				idealAngles[DIST] += cam_idealdelta.GetFloat() * ((m_nCameraY - y) / 2.0f);
			}
			if (idealAngles[DIST] > c_maxdistance.GetFloat())
			{
		...
		else if (m_nCameraY < y)
		{
			...
			{
				idealAngles[DIST] -= (cam_idealdelta.GetFloat()) * ((y - m_nCameraY) / 2.0f);
			}
			if (idealAngles[DIST] < c_mindistance.GetFloat())
			{
